...
#' @examples
#' \dontrun{
#' sim <- simulate_example_data(n_genes = 200, seed = 42)
#' result <- run_fdrreg_twas(
#'   target = sim$spredixcan_target, twas_type = "spredixcan",
...
run_fdrreg_twas <- function(target, twas_type = c("smultixcan", "spredixcan"),
  ...
  for (nm in names(aux)) {
    if (twas_type == "spredixcan") validate_columns(aux[[nm]], c(id_col, z_col), sprintf("aux$%s", nm))
    else validate_columns(aux[[nm]], c(id_col, p_col), sprintf("aux$%s", nm))
  }
  ...
  target_z <- extract_z_scores(target, z_col = z_col, p_col = p_col, random_sign = TRUE, seed = seed)

  aux_features <- if (length(aux) > 0) {
    mats <- lapply(aux, function(df) abs(extract_z_scores(df, z_col = z_col, p_col = p_col, random_sign = FALSE)))
  ...
  annot_mat <- NULL
  if (!is.null(annotations)) annot_mat <- align_annotations(target[[id_col]], annotations, id_col)
  ...
  vs_result <- perform_variable_selection(target_z, features, method = var_select, seed = seed)
  features <- vs_result$features
  covariates_used <- vs_result$selected_cols

  set.seed(seed)
  models <- fit_fdrreg_models(target_z, features, nulltype = fdrreg_nulltype, method = fdrreg_method)

  full_results <- assemble_full_results(ids = target[[id_col]], z_scores = target_z,
                                        p_values = target[[p_col]], models = models)
  thresholds <- standard_thresholds()
  sl <- list(threshold = thresholds)
  if (!is.null(models$theoretical)) sl$fdr_theo <- count_discoveries(models$theoretical$FDR, thresholds)
  if (!is.null(models$empirical)) sl$fdr_emp <- count_discoveries(models$empirical$FDR, thresholds)

  am <- if (!is.null(models$theoretical)) models$theoretical else models$empirical
  params <- record_params(seed, call)

  new_fdrreg_result(tier = paste0("twas_", twas_type), full_results = full_results,
                    summary_counts = as.data.frame(sl, stringsAsFactors = FALSE),
                    assessment = extract_model_assessment(am, covariates_used),
